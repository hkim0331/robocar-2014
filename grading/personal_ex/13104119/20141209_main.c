...
void clank(uint8_t dir) {
    if (dir == LEFT) {
        LED(LED_LEFT);
        motor(-LEFT_POWER/4,RIGHT_POWER);
    } else if (dir == RIGHT) {
        LED(LED_RIGHT);
        motor(LEFT_POWER,-RIGHT_POWER/4);
    }
}
