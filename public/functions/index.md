...
* その関数には関数の働きがわかるよう、 smaller という名前をつけよう。
* 「整数二つを受け取る」んだから、関数 smaller の引数はふたつ、ともに int だ。ふたつの整数を区別するためにそれぞれ違った名前 x, y で呼ぼう。
...
関数は{ }の中を上から下へ、Cの決まりにしたがい実行され（if で分岐したり、for でループしたり）、 
return 文に出会うか、関数定義の末尾に到着すると終了する。
...
関数 smaller を定義したら、それを呼び出すにはその名前 smaller に必要なデータ（引数）を、
()の中に与えて、
...
のように呼び出す。呼び出すというのは、その関数をコンピュータが実行するということ。

呼び出しがすんだら関数 smaller は渡した引数の小さい方の整数を返してくる。
戻り値を煮るなり焼くなりするのは関数 smaller を呼び出したプログラム（関数）の責任。
上の例では smaller(3,4) は3を返してくるが、呼び出し側はそれを受け取らず、無視している。
ま、悪いプログラム例だ。
...
c = smaller(a,b) は 「c に smaller(a,b) の評価値を代入する」。
smaller(a,b) の評価値とはsmaller(a,b)が return で返してくる値のことで、
この場合、aとbの小さい方の整数。

もっと細かく言うと、smaller(a,b)の評価値は次のように求まる。

1. 引数 a の評価値を求める。
1. 引数 b の評価値を求める。
1. それらの値を smaller の定義式に現れる引数（上の例ではx,y)に代入し、
1. smaller の { } を実行する。
1. 実行中に　return に出会ったら、その右の式を評価し、関数の戻り値（評価値）とする。
...
1,600倍の高速化に成功！どんな工夫だったんだろう？
中学生くらいなら説明すればわかるはずというくらいの単純なアイデアです。
...
* sum は定義時にすぐ0に初期化するのにiは	初期化しない理由はすぐ下で1に初期化するから。
